...
class CalculationEngine(BaseLogger):
    ...
    def __init__(self, config_manager=None, debug_mode=False, bound_calculations=None, **kwargs):
        super().__init__(**kwargs)
        self.config_manager = config_manager
        self.debug_mode = debug_mode
        
        # 获取 process_id / specification_id
        self.process_id = kwargs.get("process_id")
        if not self.process_id:
            raise WorkflowError("缺少必需参数: process_id")
        self.specification_id = kwargs.get("specification_id")
        
        # 优先使用绑定后的计算项配置
        if bound_calculations:
            self.calculations_config = bound_calculations
        else:
            # 回退到旧方式加载（向后兼容，但会逐步废弃）
    ...
    def _load_calculations_config(self) -> List[Dict[str, Any]]:
        """仅加载规范级 calculations.yaml；如缺失则返回空列表。"""
        if not self.config_manager:
            raise WorkflowError("CalculationEngine 必须通过 ConfigManager 初始化")

        # calculations.yaml 是可选的，如果加载失败，返回空列表而不是抛出异常
        if not self.specification_id:
            return []

        try:
            # 通过 ConfigManager 获取计算配置（不直接读取目录）
            calculations_config = self.config_manager.get_specification_calculations(self.specification_id)
            
            if calculations_config:
                # calculations_config 可能是字典或列表
                if isinstance(calculations_config, dict):
                    calculations = calculations_config.get("calculations", [])
                elif isinstance(calculations_config, list):
                    calculations = calculations_config
                else:
                    calculations = []
                
                if self.logger:
                    self.logger.info(f"从规范 {self.specification_id} 加载了 {len(calculations)} 个计算项")
                return calculations
            else:
                # 配置不存在，返回空列表（calculations.yaml 是可选的）
                if self.logger:
                    self.logger.debug(f"规范 {self.specification_id} 没有计算配置，将使用空计算配置")
                return []
                
        except Exception as e:
            # 任何错误都只记录警告，不抛出异常（calculations.yaml 是可选的）
            if self.logger:
                self.logger.warning(f"加载规范 {self.specification_id} 的计算配置时出错: {e}，将使用空计算配置")
            return []
        ...
        for index, calc_config in enumerate(self.calculations_config, 1):
            ...
            try:
                if calc_type == "sensor_group":
                    ...
                    # 占位符替换：用规范级 parameters 替换 {param}
                    params = calc_config.get("parameters", {})
                    if isinstance(params, dict) and params:
                        for pk, pv in params.items():
                            try:
                                formula = formula.replace(f"{{{pk}}}", str(pv))
                            except Exception:
                                pass
