...
class SpecBindingProcessor(BaseDataProcessor):
    ...
    def _load_specification_config(self, kwargs: Dict[str, Any]) -> None:
        """加载规范配置。"""
        if self.spec_index and self.rules_index:
            return  # 已经加载过了
        ...
        if not specification_id:
            # 尝试从所有规范加载（兼容模式）
            if self.logger:
                self.logger.warning("未找到 specification_id，将尝试加载所有规范")
            ...
            for spec_id in spec_ids:
                try:
                    ...
                    if self.logger:
                        self.logger.warning(f"加载规范 {spec_id} 失败: {e}")
        else:
            # 加载指定规范
            try:
                ...
                if self.logger:
                    self.logger.info(f"已加载规范配置: {specification_id}")
            except Exception as e:
                if self.logger:
                    self.logger.error(f"无法加载规范配置 {specification_id}: {e}")
                raise WorkflowError(f"无法加载规范配置 {specification_id}: {e}")
        ...
        if specification_id:
            try:
                ...
                if rules_config:
                    ...
                    if self.logger:
                        self.logger.info(f"已加载规则配置: {len(self.rules_index)} 条规则")
            except Exception as e:
                if self.logger:
                    self.logger.error(f"无法加载规则配置: {e}")
                # 不要抛出异常，允许继续执行
    ...
    def process(self, data_context: WorkflowDataContext, **kwargs: Any) -> ProcessorResult:
        ...
        try:
            ...
            # 合并 kwargs 和 data_context 中的 specification_id
            merged_kwargs = kwargs.copy()
            if 'specification_id' not in merged_kwargs and 'specification_id' in data_context:
                merged_kwargs['specification_id'] = data_context['specification_id']
            
            # 加载规范配置（如果还没有加载）
            self._load_specification_config(merged_kwargs)
    ...
    def _generate_execution_plan(self, sensor_grouping: SensorGrouping, 
        ...
        if not self.spec_index:
            ...
            # 按照规格配置生成计划
            # 注意：这里需要使用specification_id而不是material_code
