...
class DataChunker(BaseDataProcessor):
    ...
    def __init__(self, algorithm: str = "detect_stage_by_time", 
        ...
        # 不在初始化时获取配置，而是在执行时获取（支持运行时配置注入和规范配置）
        # self.stages_config 和 self.stages_index 将在 process 方法中动态获取
    ...
    def process(self, data_context: WorkflowDataContext, **kwargs: Any) -> ProcessorResult:
        ...
        try:
            ...
            # 在执行时获取阶段配置（支持运行时配置注入和规范配置）
            specification_id = kwargs.get("specification_id") or data_context.get("specification_id")
            if specification_id:
                # 从规范配置获取阶段信息
                stages_config_dict = self.config_manager.get_specification_stages(specification_id)
                if stages_config_dict:
                    stages_list = stages_config_dict.get("stages", [])
                    self.stages_index = {stage["id"]: stage for stage in stages_list}
                    # 构建完整的配置对象（包含采样间隔等全局配置）
                    self.stages_config = {
                        "stages": stages_list,
                        "sampling_interval": 0.1,  # 默认值，可以从配置中获取
                        "time_unit": "minutes"  # 默认值
                    }
                else:
                    if self.logger:
                        self.logger.warning(f"规范 {specification_id} 没有阶段配置，使用空配置")
                    self.stages_config = {"stages": [], "sampling_interval": 0.1, "time_unit": "minutes"}
                    self.stages_index = {}
            else:
                # 回退到全局配置（向后兼容）
                if self.logger:
                    self.logger.warning("未指定 specification_id，使用全局阶段配置（不推荐）")
