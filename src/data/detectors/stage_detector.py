...
class StageDetector(BaseProcessor):
    """基于时间的阶段检测器。"""
    ...
    def _load_stages_config(self, process_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            raise DataProcessingError(f"加载阶段配置失败: {e}")
    ...
    def _detect_stage_for_sensor(self, sensor_data: List[float], criteria: str, 
        ...
        for i, value in enumerate(sensor_data):
            try:
                ...
                self.logger.warning(f"传感器数据点 {i} 条件评估失败: {criteria}, 错误: {e}")
                continue
    ...
    def _filter_data_by_criteria(self, data: Dict[str, Any], criteria: str, 
        ...
        for i in range(data_length):
            try:
                ...
                self.logger.warning(f"数据点 {i} 条件评估失败: {criteria}, 错误: {e}")
                continue
    ...
    def analyze(self, data: Dict[str, Any], **kwargs: Any) -> Dict[str, Any]:
        ...
        if has_aggregated_data and not has_original_sensors:
            ...
            if original_data is None or (hasattr(original_data, 'empty') and original_data.empty):
                # 如果没有原始数据，尝试从聚合数据重构
                self.logger.warning("没有原始数据，无法进行传感器级别的阶段检测")
                return data
        ...
        # 为每个传感器创建独立的数据结构（只保存位置信息，不保存完整数据）
        sensor_data = {}
        for group_name, group_info in sensor_groups_data.items():
            for sensor_name in group_info["sensors"]:
                sensor_data[sensor_name] = {
                    "total_length": len(group_info["data"][sensor_name]),
                    "stages": {}
                }
                ...
                for stage_id, stage_info in stage_detection.items():
                    ...
                    if sensor_name in stage_info:
                        ...
                        sensor_data[sensor_name]["stages"][stage_id] = {
                            "start_index": sensor_stage_info["indices"][0] if sensor_stage_info["indices"] else None,
                            "end_index": sensor_stage_info["indices"][-1] if sensor_stage_info["indices"] else None,
    ...
    def extract_stage_data(self, sensor_name: str, stage_id: str, original_data: Dict[str, Any]) -> Dict[str, Any]:
        """根据位置信息从原始数据中提取特定阶段的数据。"""
        if "sensor_data" not in self._last_result:
            raise DataProcessingError("需要先运行阶段检测")
        
        sensor_data = self._last_result["sensor_data"]
        if sensor_name not in sensor_data:
            raise DataProcessingError(f"传感器 {sensor_name} 不存在")
        
        if stage_id not in sensor_data[sensor_name]["stages"]:
            raise DataProcessingError(f"传感器 {sensor_name} 在阶段 {stage_id} 中没有数据")
        
        stage_info = sensor_data[sensor_name]["stages"][stage_id]
        start_idx = stage_info["start_index"]
        end_idx = stage_info["end_index"]
        ...
        for key, values in original_data.items():
            if isinstance(values, list) and len(values) > end_idx:
                stage_data[key] = values[start_idx:end_idx+1]
            else:
                stage_data[key] = values
