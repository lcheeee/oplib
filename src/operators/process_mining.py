...
class TimeBasedStageDetector:
	...
	def _filter_data_by_criteria(self, data: Dict[str, Any], criteria: str, full_data: Dict[str, Any]) -> List[int]:
	...
	def run(self, data: Dict[str, Any]) -> Dict[str, Any]:
		# 获取阶段配置
		process_id = self.config.get("process_id", "curing_001")
		stages = self._load_stages_config(process_id)
		...
		# 获取传感器组配置
		sensor_groups = self._load_sensor_groups_config(process_id)
		...
		# 为每个阶段检测每个传感器的数据
		stage_detection = {}
		stage_priority = ["heating", "soaking", "cooling"]
		used_indices = set()
		
		for stage_id in stage_priority:
